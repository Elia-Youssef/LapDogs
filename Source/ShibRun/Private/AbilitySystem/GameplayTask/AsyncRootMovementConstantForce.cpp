...
#include "AbilitySystem/GameplayTask/AsyncRootMovementConstantForce.h"
#include "TimerManager.h"
#include "Engine/Engine.h"

UAsyncRootMovementConstantForce* UAsyncRootMovementConstantForce::AsyncRootMovement(const UObject* WorldContext,
	UCharacterMovementComponent* CharacterMovement, FVector WorldDirection, float Strength, float Duration,
	bool bIsAdditive, UCurveFloat* StrengthOverTime, ERootMotionFinishVelocityMode VelocityOnFinishMode,
	FVector SetVelocityOnFinish, float ClampVelocityOnFinish, bool bEnableGravity)
{
	...
	UAsyncRootMovementConstantForce* Node = NewObject<UAsyncRootMovementConstantForce>();
...
void UAsyncRootMovementConstantForce::Activate()
{
	Super::Activate();
	if (const UWorld* World = GetWorld())
    {
        ...
        if (CharacterMovement) {
            ...
            TimerManager.SetTimer(
                OngoingDelay,
                FTimerDelegate::CreateLambda([WeakThis = TWeakObjectPtr<UAsyncRootMovementConstantForce>(this)]()
            {
...
void UAsyncRootMovementConstantForce::Cancel()
{
	Super::Cancel();
